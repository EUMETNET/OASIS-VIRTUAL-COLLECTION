...
def _build_collection(base_url: str, cfg: VirtualCollectionConfig) -> Collection:
    ...
    data_queries = DataQueries(
        ...
        trajectory=_edr_query(base_url, collection_id, "trajectory"),
...
async def list_collections(request: Request) -> Collections:
    ...
    collection = _build_collection(base_url, cfg)
    return Collections(
        links=[
            Link(
...
async def get_collection(collection_id: str, request: Request) -> Collection:
    ...
    base_url = settings.api_base_url.rstrip("/")
    cfg = get_virtual_collection_config()
    return _build_collection(base_url, cfg)
